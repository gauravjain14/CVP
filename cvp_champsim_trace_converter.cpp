...
void formatToChampSim(CVPTraceReader::Instr mInstr, FILE *out) {
    ...
    */
    ...
    instr_champsim.branch_taken = (isBranch == 1) ? 0x1 : 0x0;
    ...
    for (int i=0; i<numInRegs; i++) {
        ...
        if (mInstr.mType == InstClass::condBranchInstClass)
            // is this right?
            instr_champsim.destination_registers[i] = (i%2) ? REG_FLAGS :
            ...
            instr_champsim.destination_registers[i] = mInstr.mInRegs[i];
    }
...
int main(int argc, char **argv) {
    ...
    while(reader.readInstr()) {
        //reader.mInstr.printInstr();
#ifdef CHAMPSIM_FORMAT
        formatToChampSim(reader.mInstr, out);
#endif
    }
