...
void formatToChampSim(CVPTraceReader::Instr mInstr) {
    trace_instr_format_t instr_champsim;

    instr_champsim.ip = mInstr.mPc;

    /* Where does ChampSim expect branch target address? */
    bool isBranch = (mInstr.mType == InstClass::condBranchInstClass) ||
                    (mInstr.mType == InstClass::uncondDirectBranchInstClass) ||
                    (mInstr.mType == InstClass::uncondIndirectBranchInstClass);
    instr_champsim.branch_taken = (isBranch) ? 0x1 : 0x0;

    unsigned char numInRegs = std::min(mInstr.mNumInRegs, 
                                            (uint8_t)NUM_INSTR_SOURCES);
    for (int i=0; i<numInRegs; i++) {
        instr_champsim.source_registers[i] = mInstr.mInRegs[i];
    }

    unsigned char numOutRegs = std::min(mInstr.mNumOutRegs, 
                                            (uint8_t)NUM_INSTR_DESTINATIONS);
    for (int i=0; i<numOutRegs; i++) {
        instr_champsim.destination_registers[i] = mInstr.mOutRegs[i];
    }

    /* Fill in source registers only if branch or load/store instruction?
    If yes, then only the zeroth register should be filled, right? */
    
    instr_champsim.source_memory[0] = (isBranch) ? mInstr.mTarget :
            (mInstr.mType == InstClass::loadInstClass ||
            mInstr.mType == InstClass::storeInstClass) ? mInstr.mEffAddr : 0;

    for (int i=0; i<numInRegs; i++) {
        instr_champsim.destination_memory[i] = mInstr.mOutRegsValues[i];
    }
}
